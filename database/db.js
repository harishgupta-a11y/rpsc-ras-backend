...
async function seedPlaceholderQuestionsIfNeeded() {
    try {
        ...
        for (const topic of allPreTopics) {
            ...
            for (let k = 0; k < neededHi; k++) {
                ...
                `, [
                    ...
                    `विकल्प         // 2.5 Seed custom high-quality questions for Ancient Indian History (IVC: Town Planning & Seals)
        ...
        if (mtPreEn && mtPreHi && mtMainsEn && mtMainsHi) {
            const ivcPreCount = await get("SELECT COUNT(*) as count FROM questions WHERE minute_topic_id = ?", [mtPreEn.minute_topic_id]);
            if (ivcPreCount.count === 0) {
                console.log("Seeding custom high-quality questions for IVC...");
                const customPreQuestions = [
                    {
                        lang: 'EN',
                        subtopic_id: mtPreEn.minute_topic_id,
...
D) Burnt bricks were only used for constructing drains and public baths.`,
                    ...
                    {
                        lang: 'HI',
                        subtopic_id: mtPreHi.minute_topic_id,
...
D) 1, 2 और 3`,
                ...
                const customMainsQuestions = [
                    {
                        lang: 'EN',
                        subtopic_id: mtMainsEn.minute_topic_id,
...
**Conclusion**: This dual-division indicates administrative hierarchy and class stratification.`
                    ...
                    {
                        lang: 'HI',
                        subtopic_id: mtMainsHi.minute_topic_id,
                        text: `Q. के नगर नियोजन में परिलक्षित लेआउट और सामाजिक स्तरीकरण का आलोचनात्मक परीक्षण कीजिए। [5 अंक, 50 शब्द]`,
