...
async function main() {
    try {
        const res = await client.execute(`
            SELECT question_id, question_text, option_a, option_b, option_c, option_d, correct_option, detailed_explanation 
            FROM questions 
            WHERE question_id IN (5774, 5775)
        `);
        res.rows.forEach(r => {
            console.log(`\n========================================`);
            console.log(`ID: ${r.question_id} | Correct: ${r.correct_option}`);
